...
namespace Decompiler
{
	public class ScriptFile
    {
        ...
        public ScriptFile(Stream scriptStream, bool _Console)
        {
            ConsoleVer = _Console;
            file = scriptStream;
			...
			Console.WriteLine("get static info");

			GetStaticInfo();
			Console.WriteLine("done get static info");
			Functions = new List<Function>();
            FunctionLoc = new Dictionary<int, FunctionName>();
			Console.WriteLine("get functions");
			GetFunctions();
            foreach (Function func in Functions)
            {
                func.PreDecode();
            }
			Console.WriteLine("check vars");
			Statics.checkvars();
            foreach (Function func in Functions)
            {
                func.Decode();
        ...
        void AddFunction(int start1, int start2)
        {
            ...
            {
				Console.WriteLine("CodeTable[{0}]: {1:X}", temp, CodeTable[temp]);
        ...
        private void GetStaticInfo()
        {
            ...
            reader.BaseStream.Position = Header.StaticsOffset;
